...
Result<FileMapping, std::error_code> FileMapping::from(std::string_view path) {
	...
	HANDLE file_handle = ::CreateFileW(w_path.data(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
									   FILE_ATTRIBUTE_NORMAL, nullptr);
	if (file_handle == INVALID_HANDLE_VALUE) {
		return get_system_error();
	}
	UniqueHandle file(file_handle);
	...
	if (size > 0) {
		...
		LPVOID map_addr = ::MapViewOfFile(mapping.get(), FILE_MAP_READ, 0, 0, 0);
		if (map_addr == nullptr) {
			return get_system_error();
		}
		addr.reset(map_addr);
